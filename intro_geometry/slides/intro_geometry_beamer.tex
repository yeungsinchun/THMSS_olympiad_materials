...
\begin{frame}{A brain teaser}
	\begin{columns}
		...
		\begin{tikzpicture}
			...
			\tkzAutoLabelPoints[center=E,inner sep=2.5mm](A,B,C,D)
...
\begin{frame}{Inscribed Angle Theorem (Proof for case I)}
	\begin{columns}
		\column{0.5\textwidth}
		\hfill
		\begin{tikzpicture}[scale=1.5,radius=1.5cm,a/.style={magenta},b/.style={blue}]
			\coordinate[label=$O$] (O) at (0,0);
			\draw (O) circle;
			\draw (O) -- ++(30:1.5cm) coordinate[label=30:$A$](A);
			\draw (O) -- ++(-50:1.5cm) coordinate[label=300:$B$](B);
...
\begin{frame}{The Incenter/Excenter Lemma}
	Now, we'll demonstrate an interesting lemma by introducing the tool Geogebra.	
	\begin{theorem}[The Incenter/Excenter Lemma]
		Let $ABC$ be a triangle with incenter $I$. Ray $AI$ meet $(ABC)$ again at $L$. Let $I_A$	 be the reflection over $L$. Then,
		\begin{enumerate}
			\item The points  $I,B,C,$ and  $I_A$ lie on a circle with diameter  $\overline{II_A}$ with center $L$. In particular
			\item Rays $BI_A$ and  $CI_A$ bisect the exterior angles of  $\triangle ABC$.
		\end{enumerate}
	\end{theorem}
\end{frame}
...
\begin{frame}{Example Problem 1}
	\begin{block}{Question 1}
		(Prelim 2020 Q6) In $\Delta ABC, AB = 6, BC = 7, CA = 8$. Let $D$ be the mid-point of minor arc $AB$ on the circumcircle of $\Delta ABC$. Find $AD^2$.
	\end{block}
	\pause
	...
	\begin{columns}
		...
		\begin{center}
			\begin{tikzpicture}[scale=0.9]
				...
				\tkzDrawSegment(A,C)
				\tkzInterLC(A,C)(O,R) \tkzGetPoints{N}{M}
				\tkzLabelPoint[anchor=255,small label](N){N}
...
\begin{frame}{The Radical Axis and Radical Center (Supplementary) }
	Three circles intersect as in below.
	\begin{figure}
		\begin{tikzpicture}[scale=0.5]
			...
			\tkzDefPoint(2.2,0){O3}
			\tkzDrawCircle[R](O1,2)
			\tkzDrawCircle[R](O2,2.7)
			\tkzDrawCircle[R](O3,2.5)
			\tkzInterCC[R](O1,2)(O2,2.7) \tkzGetPoints{M1}{M2}
			\tkzDrawSegment(M1,M2)
			\tkzInterCC[R](O2,2.7)(O3,2.5) \tkzGetPoints{M3}{M4}
			\tkzDrawSegment(M3,M4)
			\tkzInterCC[R](O1,2)(O3,2.5) \tkzGetPoints{M5}{M6}
			\onslide<2->{
				\tkzDrawSegment[red](M5,M6)
			}
		\end{tikzpicture}
	\end{figure}
	Are the chords necessarily concurrent?
\end{frame}

\begin{frame}{The Radical Axis}
	\begin{definition} [Radical Axis]
		Given two circles $\omega_1$ and $\omega_2$ with distinct centers, the radical axis of the circles is the set of points $P$ such that 
		\begin{equation*}
			\Pow_{\omega_1} (P) = \Pow_{\omega_2}(P)
		\end{equation*}
	\end{definition}
	\begin{theorem}[Radical Axis]
		Let $\omega_1$ and $\omega_2$ be circles with distinct centers $O_1$ and $O_2$. The radical axis of $\omega_1$ and $\omega_2$ is a straight line perpendicular to $\overline{O_1O_2}$. \\
		In particular, if $\omega_1$ and $\omega_2$ intersect at two points $A$ and $B$, then the radical axis is $AB$.
	\end{theorem}
	\begin{center}
		\begin{tikzpicture}[scale=0.4,small label]
			\def\ra{2}
			\def\rb{2.5}
			\tkzDefPoint(0,0){O1}
			\tkzDefPoint(3,0){O2}
			\tkzDrawCircle[R](O1,\ra)
			\tkzDrawCircle[R](O2,\rb)
			\tkzInterCC[R](O1,\ra)(O2,\rb) \tkzGetPoints{A}{B}
			\tkzDrawLine[dashed,<->](A,B)
			\tkzLabelPoints[small label, right](A){A}
			\tkzLabelPoints[small label, right](B){B}
		\end{tikzpicture}
		\begin{tikzpicture}[scale=0.4,small label]
			\def\ra{1}
			\def\rb{2}
...
\begin{frame}{Proof of concurrent chords}
	...
	\begin{theorem}[Converse of radical center]
